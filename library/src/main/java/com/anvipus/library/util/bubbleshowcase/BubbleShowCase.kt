...
class BubbleShowCase(builder: BubbleShowCaseBuilder){
    ...
    private val mHasNavBar: Boolean = builder.mHasNavBar!!
    private val mNavBarHeight: Int = builder.mNavBarHeight!!
    ...
    private val mShowcaseSize: Int? = builder.mShowcaseSize
    private val mShowcasePosition: Int? = builder.mShowcasePosition
    ...
    private fun getBubbleMessageViewBuilder(): BubbleMessageView.Builder{
        return BubbleMessageView.Builder()
                ...
                .title(mTitle)
                .showCaseSize(mShowcaseSize)
                .showCasePosition(mShowcasePosition)
                .subtitle(mSubtitle)
                .image(mImage)
                ...
                .listener(object : OnBubbleMessageViewListener {
                    ...
                    override fun onSkipButtonClick() {
                        dismiss()
                        finishSequence()
                        mBubbleShowCaseListener?.onSkipClick(this@BubbleShowCase)
                    }
                })
    }
            ...
            ArrowPosition.BOTTOM -> {
                ...
                if(ScreenUtils.isViewLocatedAtHalfLeftOfTheScreen(mActivity.get()!!, targetView)){
                    showCaseParams.setMargins(
                            ...
                            if (isTablet()) getScreenWidth(mActivity.get()!!) - getXposition(targetView) - getMessageViewWidthOnTablet(getScreenWidth(mActivity.get()!!) - getXposition(targetView)) else 0,
                            if (mHasNavBar) {
                                getScreenHeight(mActivity.get()!!) - getYposition(targetView) + mNavBarHeight
                            } else {

package com.spinlibrary.util.bubbleshowcase
...
class BubbleMessageView : ConstraintLayout {
    ...
    private var btnSkipShowcase: TextView? = null
    private var textViewTitle: TextView? = null
    private var textViewSubtitle: TextView? = null
    private var btnNextShowcase: TextView? = null
    ...
    private fun bindViews() {
        btnSkipShowcase = findViewById(R.id.btn_skip_showcase)
        btnNextShowcase = findViewById(R.id.btn_next_showcase)
        ...
        layoutIndicator = findViewById(R.id.layout_showcase_indicator)
    }

    private fun setAttributes(builder: Builder){
        if(builder.mImage!=null){
            btnSkipShowcase?.visibility = View.VISIBLE
        }
        if(builder.mCloseAction!=null){
            btnNextShowcase?.visibility = View.VISIBLE
        }

        if(builder.mDisableCloseAction!=null && builder.mDisableCloseAction!!){
            btnNextShowcase?.visibility = View.INVISIBLE
        }

        if (builder.mShowcaseSize != null && builder.mShowcasePosition != null) {
            if (builder.mShowcaseSize!! > 1) {
                setupIndicator(layoutIndicator!!, builder.mShowcaseSize!!, builder.mShowcasePosition!!)
            }

            if (builder.mShowcasePosition == builder.mShowcaseSize!! - 1) {
                btnSkipShowcase?.isVisible = false
                btnNextShowcase?.text = "Mengerti"
            }
        }
    ...
    private fun setBubbleListener(builder: Builder){
        btnNextShowcase?.setOnClickListener { builder.mListener?.onCloseActionImageClick() }
        btnSkipShowcase?.setOnClickListener { builder.mListener?.onSkipButtonClick() }
        itemView?.setOnClickListener {builder.mListener?.onBubbleClick()}
    }

    private fun setupIndicator(container: LinearLayout, showCaseSize: Int, showCasePosition: Int) {
        fun ImageView.indicatorDrawable(): ImageView {
            ...
            layoutParams = params
            setBackgroundResource(R.drawable.vp_indicator)
            return this
        }

        container.removeAllViews()
        repeat((0 until showCaseSize).count()) {
            val inc = ImageView(context)
            container.addView(inc.indicatorDrawable())
        }

        container.getChildAt(showCasePosition).isSelected = true
    }
    ...
    class Builder{
        ...
        var mShowcaseSize: Int? = null
        var mShowcasePosition: Int? = null
        ...
        fun showCaseSize(showCaseSize: Int?): Builder {
            mShowcaseSize = showCaseSize
            return this
        }

        fun showCasePosition(showCasePosition: Int?): Builder {
            mShowcasePosition = showCasePosition
            return this
        }
